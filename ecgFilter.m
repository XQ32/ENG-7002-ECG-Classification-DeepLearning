...
% ecgFilter - Filters an ECG signal using various filtering methods
% 
% Input:
%   ecg - Raw ECG signal (can be a row or column vector)
...
if power_line_freq ~= 50 && power_line_freq ~= 60
    warning('Power line frequency parameter %d Hz is invalid, only 50Hz or 60Hz are supported. Using default value 60Hz.', power_line_freq);
    power_line_freq = 60;
end

% Set cutoff frequency to 0.5Hz
cutoff_freq = 0.5;
...
if fs > power_line_freq * 3 % Ensure the sampling rate is high enough to filter out harmonics
    % Add 2x power line frequency filter
    wo_2nd = (power_line_freq*2)/(fs/2);
    if wo_2nd < 0.95 % Ensure normalized frequency is less than 1
        bw_2nd = wo_2nd/40; % Make harmonic filter bandwidth narrower
...
%% Step 2: Select the method for baseline wander removal based on method_index
switch method_index
    case 1
    ...
    case 2
        ...
        try
            ...
            warning('Error occurred while executing Method 2 (IIR): %s, using Method 1 as a fallback', e.message);
            [ecg_baseline_filtered, filter_params] = apply_FIR_filter(ecg_notch_filtered, fs, cutoff_freq);
...
% Low-pass filter cutoff frequency is set to 150Hz to remove high-frequency noise while preserving main ECG components
...
if fs > 2 * lowpass_cutoff % Ensure the sampling rate meets the Nyquist theorem
    ...
    % Skip low-pass filtering if the sampling rate is too low
    ecg_filtered = ecg_baseline_filtered;
    filter_info.lowpass_applied = false;
    warning('Sampling rate %d Hz is too low, skipping %d Hz low-pass filter', fs, lowpass_cutoff);
end
...
function [filtered_signal, params] = apply_FIR_filter(signal, fs, cutoff_freq)
    ...
    mags = [0 1];
    % Increase the error tolerance to reduce the required filter order
    devs = [0.01 0.005];  % Relax the error requirements
